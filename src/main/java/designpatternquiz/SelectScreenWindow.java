...
public class SelectScreenWindow extends JFrame {
	...
	public SelectScreenWindow() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(SelectScreenWindow.class.getResource("/designpatternquiz/512px-Eo_circle_pink_checkmark.svg.png")));
		...
		welcomeMessageLabel.setBounds(10, 11, 478, 33);
		...
		final JRadioButton selectRangeRadioButton = new JRadioButton("I only want a few questions!  (Select a range from 1-10)");
				...
				if (e.getSource() == startQuizButton) {
					if(selectRangeRadioButton.isSelected() == true) {
						if(isInputCorrect(fromTextField, toTextField) == false && allQuestionsRadioButton.isSelected() == false) {
							JOptionPane.showMessageDialog(null, "You must enter numbers for the question you want to be tested on in correct order (1 -10)");
						} else {
							EventQueue.invokeLater(new Runnable() {
								public void run() {
									try {
		...
		lblNewLabel.setIcon(new ImageIcon(SelectScreenWindow.class.getResource("/designpatternquiz/pink-romantic-background.jpg")));
		lblNewLabel.setBounds(0, 0, 488, 241);
	...
	//better implementation: create an Exam object and a getNumberOfQuestions method, instead of writing 10
